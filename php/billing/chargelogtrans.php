...
$where = ' WHERE TRUE  AND t.billing_profile_id NOT IN (4,6,7,8,9,19,22,23,26,27, 28, 29, 32, 36)';

$where .= "  AND  d.created_at >= '$log_start' AND created_at <= '$log_end' ";

$where .= " AND t.account_id = '$billing_id' ";

if ($chargelogtrans != '' && $chargelogtrans != 'ALL') {

	$where.=" AND p.name = '$chargelogtrans' ";
	//$offset = 0;
}
...
$sql ="SELECT t.id as paymentid, t.committed as committed , t.billing_profile_id, to_char(min(d.created_at), 'YYYY-MM-DD HH24:MI:SS') as start , to_char(max(d.created_at), 'YYYY-MM-DD HH24:MI:SS') as end, min(d.meta_data) as meta_data, max(p.name) as name, min(r.result_name) as result_name FROM b_transactions t JOIN b_transactions_detail d ON d.transaction_id = t.id JOIN b_billing_profile p ON t.billing_profile_id = p.id JOIN b_transaction_result r ON r.id = d.result   $where GROUP BY t.id order by t.id DESC OFFSET $offset LIMIT $limit";
/*print_r($sql);
exit;*/
